...
solve(WaterDrinker, ZebraOwner) :-
  ...
  adjacent(house(_, _, _, _, birkenstock_shoes), house(_, _, fox, _, _), Houses),   % 11. The man who smokes Chesterfields lives in the house next to the man with the fox.
  adjacent(house(_, _, _, _, rubbersole_shoes), house(_, _, horse, _, _), Houses),  % 12. Kools are smoked in the house next to the house where the horse is kept.
